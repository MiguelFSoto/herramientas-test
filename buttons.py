...
def renderGrid(player):
    ...
    for x in range(10):
        for y in range(10):
            ...
            # Si hay un barco, pinta el label de negro
            if oceanTile.value in range(5, 10):
                player.oceanLabels[x][y].configure(background="black")
            # Si hay un barco impactado, pinta el label de rojo
            elif oceanTile.value in range(10, 15):
                player.oceanLabels[x][y].configure(background="red")
            # Si hay un disparo fallido, pinta el label de gris
            elif oceanTile == TileStates.MissedShot:
                player.oceanLabels[x][y].configure(background="gray")
            ...
            if targetTile == TileStates.MissedShot:
                player.targetLabels[x][y].configure(background="gray")
            # Si hay un disparo acertado, pinta el label de rojo
            elif targetTile == TileStates.HitShot:
                player.targetLabels[x][y].configure(background="red")
...
def prepareNextPhase(player1, player2):
    ...
    if player1.placedShips == 5 and player2.placedShips == 5:
        ...
        infoLabel.configure(text = "Fase de disparos") 
...
def gameInit():
    ...
    infoLabel.configure(text="Fase de ubicacion: Carrier")
